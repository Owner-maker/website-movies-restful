document.addEventListener('click', ({ target: t }) => {
    if (t.tagName === 'INPUT' && t.disabled===false) {
        if(t.alt.startsWith("del")){
            ...
            let response = fetch('http://localhost:8080/movies/deletemovie/'+t.alt, {
                method: 'DELETE',
                mode: 'cors',
                cache: 'no-cache',
            ...
            localStorage.setItem("editingFilmId",t.alt);
            window.location.href = "http://localhost:8080/html/admin/filmEdit.html";
        }
    }
});
