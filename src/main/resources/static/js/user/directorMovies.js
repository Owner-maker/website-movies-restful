...
document.addEventListener("DOMContentLoaded", async function() {
    let directorId = localStorage.getItem("DirectorIdToSeeMovies");
    let token = 'Bearer ' + localStorage.getItem('token');

    let response = await fetch('http://localhost:8080/movies/getdirector/'+directorId, {
        method: 'GET',
        mode: 'cors',
        cache: 'no-cache',
    ...
    if(response.ok){
        let director = await response.json();
        let result = director.movies;
        let library = [];
        if(localStorage.getItem("library")!== null){
